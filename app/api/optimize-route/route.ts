...
async function placesNearestByName(start: Geo, name: string): Promise<Geo> {
  ...
  // 2) fallback: radius 50km + closest by haversine
...
export async function POST(req: Request) {
  try {
    ...
    if (!GOOGLE_PLACES_KEY) return NextResponse.json({ optimized: false, note: "Missing GOOGLE_PLACES_API_KEY" }, { status: 500 });
